...
public class ReminderController {
	...
	public ResponseEntity<?> createReminder(@RequestBody Reminder reminder, HttpSession session) {
		...
		if (null != session && null != session.getAttribute(SESSION_ID)) {
			...
			if (this.reminderService.createReminder(reminder)) {
				...
				return new ResponseEntity<String>("Reminder created Successfully", headers, HttpStatus.CREATED);

			} else {
				return new ResponseEntity<String>("Reminder created Successfully", headers, HttpStatus.CONFLICT);
			}
		} else {
	...
	public ResponseEntity<?> createReminder(@PathVariable int id, HttpSession session) {
		HttpHeaders headers = new HttpHeaders();

		if (null != session && null != session.getAttribute(SESSION_ID)) {
			if (this.reminderService.deleteReminder(id)) {

				return new ResponseEntity<String>("Reminder deleted Successfully", headers, HttpStatus.OK);

			} else {
				return new ResponseEntity<String>("Reminder deleted Successfully", headers, HttpStatus.NOT_FOUND);
			}
		} else {
			return new ResponseEntity<String>("User is Unauthorized", headers, HttpStatus.UNAUTHORIZED);
		}
	...
	public ResponseEntity<?> updateReminder(@RequestBody Reminder reminder, HttpSession session) {
		...
		if (null != session && null != session.getAttribute(SESSION_ID)) {
			...
			try {
				reminder.setReminderCreatedBy(userId);
				reminder = this.reminderService.updateReminder(reminder, reminder.getReminderId());
				if (null != reminder) {
					return new ResponseEntity<Reminder>(reminder, headers, HttpStatus.OK);
				} else {
					return new ResponseEntity<String>("Reminder updated Successfully", headers, HttpStatus.NOT_FOUND);
				}
	...
	public ResponseEntity<?> getReminder(HttpSession session) {
		...
		if (null != session && null != session.getAttribute(SESSION_ID)) {
			...
			return new ResponseEntity<List<Reminder>>(reminderList, headers, HttpStatus.OK);
