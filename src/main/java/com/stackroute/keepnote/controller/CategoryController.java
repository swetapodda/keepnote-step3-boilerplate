...
public class CategoryController {
	...
	public ResponseEntity<?> createCategory(@RequestBody Category category, HttpSession session) {
		...
		if (null != session && null != session.getAttribute(SESSION_ID)) {
			...
			if (this.categoryService.createCategory(category)) {
				...
				return new ResponseEntity<String>("Category created Successfully", headers, HttpStatus.CREATED);

			} else {
				return new ResponseEntity<String>("Category created Successfully", headers, HttpStatus.CONFLICT);
			}
		} else {
	...
	public ResponseEntity<?> createCategory(@PathVariable int id, HttpSession session) {
		HttpHeaders headers = new HttpHeaders();

		if (null != session && null != session.getAttribute(SESSION_ID)) {
			if (this.categoryService.deleteCategory(id)) {

				return new ResponseEntity<String>("Category created Successfully", headers, HttpStatus.OK);

			} else {
				return new ResponseEntity<String>("Category created Successfully", headers, HttpStatus.NOT_FOUND);
			}
		} else {
			return new ResponseEntity<String>("User is Unauthorized", headers, HttpStatus.UNAUTHORIZED);
		}
	...
	public ResponseEntity<?> updateCategory(@RequestBody Category category, HttpSession session) {
		...
		if (null != session && null != session.getAttribute(SESSION_ID)) {
			...
			try {
				category.setCategoryCreatedBy(userId);
				category = this.categoryService.updateCategory(category, category.getCategoryId());
				if(null !=category) {
					return new ResponseEntity<Category>(category, headers, HttpStatus.OK);
				}else {
					return new ResponseEntity<String>("Category updated Successfully", headers, HttpStatus.NOT_FOUND);
				}				
