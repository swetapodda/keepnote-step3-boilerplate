	...
	public ResponseEntity<String> login(HttpSession session, @RequestBody User user) {
		...
		try {
			if (userService.validateUser(user.getUserId(), user.getUserPassword())) {
				...
				return new ResponseEntity<String>("User authenticated", headers, HttpStatus.OK);
			}
			return new ResponseEntity<String>("Unable to authenticate user", headers, HttpStatus.UNAUTHORIZED);
